...
function clickBtn (){
  ...
  for(let i = 0; i < cards.length; i++){
    myCards.push(+cards[i]['id'])
  }
  for(let i = 0; newCards.length < myCards.length; i++){
    let num = Math.floor(Math.random() * 10)
    if(num <= 7 && myCards.indexOf(num) === -1 && newCards.indexOf(num) === -1){
      newCards.push(num);
    }
  }
  ...
  for(let i = 0; i < newCard.length; i++){
    newCard[i].id = newCards[i];
    newCard[i].innerHTML = `
    <div class='card-img'><img class='images' src='${pets[newCards[i]]["img"]}' alt='image'></div>
    <div class='card-title'>${pets[newCards[i]]["name"]}</div>
